...
import type { QueryLoader } from './types'
import type { InferArgs } from '../engine/makeQueryLoader'
...
export type GenerateMockOptions<
  TLoader extends QueryLoader,
  TWhere = InferArgs<TLoader> extends { where?: infer TWhere }
    ? Omit<TWhere, 'AND' | 'OR' | 'NOT'>
    : never
> = {
  zodMappers?: {
    [key in keyof typeof zod]?: (
      field: string,
      zodType: zod.ZodTypeAny
    ) => (typeof zod)[key] extends zod.ZodType<infer TOutput> ? TOutput : any
  }
  mappers?: {
...
export function mockZod(
  field: zod.ZodTypeAny,
  options?: GenerateMockOptions<any> & { fieldName?: string; counter?: number }
): any {
  ...
  if (field instanceof zod.ZodNumber) {
    ...
    return Math.random() > 0.5
      ? [mockZod((field._def as any).type, options)]
      : []
    ...
    field instanceof zod.ZodNullable ||
    field instanceof zod.ZodOptional
  ) {

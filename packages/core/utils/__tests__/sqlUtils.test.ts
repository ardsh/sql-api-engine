...
describe('Filters', () => {
  ...
  it('Array filter', () => {
    ...
    expect(arrayFilter('a', sql.fragment`test`)).toEqual(
      sql.fragment`test = ANY(${sql.array(['a'], 'text')})`
    )
    expect(arrayFilter(['a', 'b'], sql.fragment`test`)).toEqual(
      sql.fragment`test = ANY(${sql.array(['a', 'b'], 'text')})`
    )
  })
  it('Date filter', () => {
    ...
    expect(dateFilter({ _gt: '2021-01-01' }, sql.fragment`test`)).toEqual(
      sql.fragment`(test > ${'2021-01-01'})`
    )
    expect(dateFilter({ _lt: '2021-01-01' }, sql.fragment`test`)).toEqual(
      sql.fragment`(test < ${'2021-01-01'})`
    )
    expect(dateFilter({ _gte: '2021-01-01' }, sql.fragment`test`)).toEqual(
      sql.fragment`(test >= ${'2021-01-01'})`
    )
    expect(dateFilter({ _lte: '2021-01-01' }, sql.fragment`test`)).toEqual(
      sql.fragment`(test <= ${'2021-01-01'})`
    )
    expect(
      dateFilter({ _gt: '2021-01-01', _lt: '2021-01-02' }, sql.fragment`test`)
    ).toEqual(
      sql.fragment`(test > ${'2021-01-01'}) AND (test < ${'2021-01-02'})`
    )
  })
  it('Boolean filter', () => {
    expect(booleanFilter(null, sql.fragment`test`)).toBeNull()
    expect(booleanFilter(undefined, sql.fragment`test`)).toBeNull()
    expect(booleanFilter(true, sql.fragment`true statement`)).toEqual(
      sql.fragment`true statement`
    )
    expect(booleanFilter(false, sql.fragment`true statement`)).toEqual(
      sql.fragment`NOT ( true statement )`
    )
  })
  ...
  it('Comparison filter', () => {
    ...
    expect(
      comparisonFilter(
        comparisonFilterType.parse({ _gt: '1' }),
        sql.fragment`test`
      )
    ).toEqual(sql.fragment`(test > ${'1'})`)
    expect(
      comparisonFilter(
        comparisonFilterType.parse({ _lt: '1' }),
        sql.fragment`test`
      )
    ).toEqual(sql.fragment`(test < ${'1'})`)
    expect(
      comparisonFilter(
        comparisonFilterType.parse({ _gte: '1' }),
        sql.fragment`test`
      )
    ).toEqual(sql.fragment`(test >= ${'1'})`)
    expect(
      comparisonFilter(
        comparisonFilterType.parse({ _lte: '1' }),
        sql.fragment`test`
      )
    ).toEqual(sql.fragment`(test <= ${'1'})`)
    expect(
      comparisonFilter(
        comparisonFilterType.parse({ _neq: '1' }),
        sql.fragment`test`
      )
    ).toEqual(sql.fragment`(test != ${'1'})`)
    expect(
      comparisonFilter(
        comparisonFilterType.parse({ _eq: '1' }),
        sql.fragment`test`
      )
    ).toEqual(sql.fragment`(test = ${'1'})`)
    expect(
      comparisonFilter(
        comparisonFilterType.parse({ _in: ['1', '2'] }),
        sql.fragment`test`
      )
    ).toEqual(sql.fragment`(test = ANY(${sql.array(['1', '2'], 'text')}))`)
    expect(
      comparisonFilter(
        comparisonFilterType.parse({ _nin: ['1', '2'] }),
        sql.fragment`test`
      )
    ).toEqual(
      sql.fragment`(NOT ( test = ANY(${sql.array(['1', '2'], 'text')}) ))`
    )
    expect(
      comparisonFilter(
        comparisonFilterType.parse({ _is_null: true }),
        sql.fragment`test`
      )
    ).toEqual(sql.fragment`(test IS NULL)`)
    expect(
      comparisonFilter(
        comparisonFilterType.parse({ _is_null: false }),
        sql.fragment`test`
      )
    ).toEqual(sql.fragment`(test IS NOT NULL)`)
  })
  ...
  it('String filter', () => {
    ...
    expect(
      stringFilter(stringFilterType.parse('textString'), sql.fragment`test`)
    ).toEqual(sql.fragment`(test = ${'textString'})`)
    expect(
      stringFilter(
        stringFilterType.parse({ _in: ['1', '2'] }),
        sql.fragment`test`
      )?.sql
    ).toMatch(sql.fragment`(test = ANY(${sql.array(['1', '2'], 'text')}))`.sql)
    expect(
      stringFilter(stringFilterType.parse({ _like: '1' }), sql.fragment`test`)
    ).toEqual(sql.fragment`(test LIKE ${'1'})`)
    expect(
      stringFilter(stringFilterType.parse({ _ilike: '1' }), sql.fragment`test`)
    ).toEqual(sql.fragment`(test ILIKE ${'1'})`)
    expect(
      stringFilter(stringFilterType.parse({ _nlike: '1' }), sql.fragment`test`)
    ).toEqual(sql.fragment`(test NOT LIKE ${'1'})`)
    expect(
      stringFilter(stringFilterType.parse({ _nilike: '1' }), sql.fragment`test`)
    ).toEqual(sql.fragment`(test NOT ILIKE ${'1'})`)
    expect(
      stringFilter(stringFilterType.parse({ _regex: '1' }), sql.fragment`test`)
    ).toEqual(sql.fragment`(test ~ ${'1'})`)
    expect(
      stringFilter(stringFilterType.parse({ _iregex: '1' }), sql.fragment`test`)
    ).toEqual(sql.fragment`(test ~* ${'1'})`)
    expect(
      stringFilter(stringFilterType.parse({ _nregex: '1' }), sql.fragment`test`)
    ).toEqual(sql.fragment`(test !~ ${'1'})`)
    expect(
      stringFilter(
        stringFilterType.parse({ _niregex: '1' }),
        sql.fragment`test`
      )
    ).toEqual(sql.fragment`(test !~* ${'1'})`)
  })
...
describe('Query builders', () => {
  ...
  it('Rows to array', () => {
    expect(rowsToArray(sql.fragment`test FROM`, 'test')?.sql).toMatch(
      'json_agg'
    )
  })
...
describe('jsonbFilter', () => {
  ...
  it('handles nested object with mixed types', () => {
    ...
    expect(result?.sql).toBe(
      `(("meta"->'nested'->>'foo') = $1 AND ("meta"->'nested'->>'bar')::float8 = $2 AND (("meta"->'nested'->'deep'->>'baz')::float8 = $3 AND ("meta"->'nested'->'deep'->>'qux')::bool = $4))`
    )
  ...
  it("Doesn't ignore null values in nested fields", () => {
    ...
    const result = jsonbFilter('meta', nestedObject)
    expect(result?.sql).toBe(
      `(("meta"->'nested'->>'bar')::float8 = $1 AND ("meta"->'nested'->>'foo') IS NULL)`
    )

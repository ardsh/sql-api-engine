...
import { makeQueryLoader, createOptions } from '../engine/index'
import { rowToJson } from '../utils/sqlUtils'
...
`
const view = buildView`FROM test_table_bar
LEFT JOIN users
...
export const testLoader = async (
  opts?: { name?: string } & Omit<
    Parameters<typeof makeQueryLoader>[0],
    'query' | 'db'
  >
) => {

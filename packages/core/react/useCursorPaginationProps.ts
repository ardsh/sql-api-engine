...
export function cursorPaginationReducer(
  state: CursorPagination = initialCursorPagination,
  ...
  switch (action.type) {
            ...
            currentPage:
              state.currentPage !== null
                ? Math.max(
                    1,
                    (state.currentPage || 1) -
                      Math.floor(state.skip / (state.pageSize || 25))
                  )
                : null
          }),
        loading: false,
    ...
    case 'NEXT_PAGE':
      return {
        ...state,
        currentPage:
          state.currentPage !== null && state.currentCursor !== state.endCursor
            ? Math.max(
                1,
                (state.currentPage || 1) + 1 + (action.skipPages || 0)
              )
            : null,
...
export const useCursorPaginationActions = (

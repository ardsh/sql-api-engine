...
import {
  ...
  useCursorPaginationActions
} from './useCursorPaginationProps'
...
export const makeTableCreator = <
  TColumnDefinitions extends Record<string, any>
>(creatorOptions?: {
  useFilterColumns?: (
    columns: TColumnDefinitions[],
    ctx: any
  ) => TColumnDefinitions[]
}) => {
  return function tableDataLoader<TPayload>() {
    const reducer = (state: typeof initialState, action: Action) => {
      switch (action.type) {
        ...
        case 'TABLE_CHANGE':
          return {
            ...state,
            cursorPagination: cursorPaginationReducer(
    ...
    const SelectContext = React.createContext(
      [] as Exclude<keyof TPayload, number | symbol>[]
    )
    ...
    const StateRefContext = React.createContext({
      current: initialState
    } as React.RefObject<typeof initialState>)
    const DataContext = React.createContext({
      current: initialData
    } as React.MutableRefObject<typeof initialData>)
    ...
    type ExtraColumnFields = {
      select?:
        | ((ctx?: any) => readonly (keyof TPayload)[])
    ...
    return {
      ContextProvider,
      CountPopover: (props: {
        showTotal: (count?: number) => React.ReactNode
        showTotalPopover: (activate: () => any) => React.ReactNode
      }) => {
        const [displayed, setDisplayed] = React.useState(false)
        const dispatch = React.useContext(DispatchContext)
        const dataRef = React.useContext(DataContext)
        const count =
          dataRef.current?.pageInfo?.count ||
          dataRef.current?.pageInfo?.minimumCount
        return (
          (displayed
            ? props.showTotal(count)
            : props.showTotalPopover(() => {
                setDisplayed(true)
                dispatch({
                  type: 'TAKE_COUNT',
                  takeCount: true
      ...
      useCursorPaginationProps: (
        props: Omit<CursorPaginationProps, 'initialPagination' | 'tableKey'>
      ) => {
        const dispatch = React.useContext(DispatchContext)
        const tableKey = React.useContext(TableKeyContext)
        const pagination = React.useContext(CursorContext)

        const actions = useCursorPaginationActions(
          dispatch,
          props.onPageChange || noop,
          props.onPageSizeChange || noop
        )

        const getPaginationProps = React.useCallback(
          (overwrite?: (pagination: CursorPagination) => CursorPagination) => {
            return {
              ...pagination,
      ...
      withDataLoader<TProps extends { children: React.ReactNode }>(
        WrappedComponent: React.ComponentType<TProps>,
        options?: Omit<ContextProps, 'children'>
      ) {
        ...
        ComponentWithContext.displayName = `withDataLoader(${
          WrappedComponent.displayName || WrappedComponent.name || 'Component'
        })`
        return ComponentWithContext
      },
      useColumns: <TContext extends any>(
        columns: (Omit<TColumnDefinitions, 'render' | 'select'> &
          ExtraColumnFields)[],
        ctx?: TContext
      ) => {
        const tableKey = React.useContext(TableKeyContext)
        const finalColumns =
          (creatorOptions?.useFilterColumns?.(columns as any, {
            ...(ctx as any),
            tableKey
          }) as (TColumnDefinitions & {
            select:
              | ((ctx: TContext) => Extract<keyof TPayload, string>)
        ...
        React.useEffect(() => {
          const select = finalColumns
            .flatMap(column => {
              const select =
                typeof column.select === 'function'
                  ? column.select(ctxBox.current as any)
                  : column.select
              return select || []
            })
            .filter(Boolean)
      ...
      useSelectFields<TSelect extends Exclude<keyof TPayload, number | symbol>>(
        select: TSelect[]
      ) {
        const dispatch = React.useContext(DispatchContext)
        const selected = React.useContext(SelectContext)
      ...
      useDataGetter: <
        TSelect extends Exclude<keyof TPayload, number | symbol> = never
      >(
        select?: TSelect[]
      ) => {
        ...
        return (): Pick<Required<TPayload>, TSelect>[] =>
          Array.isArray(data.current)
            ? data.current
            : (data.current?.nodes as never)
      },
      useDataFragment: <
        TSelect extends Exclude<keyof TPayload, number | symbol> = never
      >(
        ...
        | ([TSelect] extends [never]
            ? TPayload
            : Pick<Required<TPayload>, TSelect>)
        | undefined => {
        const data = React.useContext(DataContext)
        const selected = React.useContext(SelectContext)
        React.useEffect(() => {
        ...
        return Array.isArray(data.current)
          ? data.current?.find(item => item.id === id)
          : data.current?.nodes?.find((item: any) => item.id === id)
      },
      ...
      createColumn: <
        TSelect extends Exclude<keyof TPayload, symbol | number> = never
      >(
        arg: Omit<TColumnDefinitions, 'render' | 'select'> & {
          select?: ((ctx?: any) => TSelect[]) | TSelect[]
          render?: (
            data: Required<Pick<TPayload, TSelect>>,
            ...args: any[]
          ) => React.ReactNode
        }
      ) => {

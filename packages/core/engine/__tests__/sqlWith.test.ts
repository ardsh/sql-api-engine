...
const newUsers = [{
    ...
    date_of_birth: new Date(Date.now() - 1000 * 60 * 60 * 24 * 365 * 20 * Math.random()).toISOString(),
    created_at: new Date().toISOString(),
}];

it("Inserts data in tables", async () => {
    const data = await sqlWith(newUsers)
        .runDB((users) => sql.unsafe`INSERT INTO users (id, first_name, last_name, email, date_of_birth, created_at)
    ...
    const newData = await db.any(sql.unsafe`SELECT id, first_name, last_name FROM users WHERE id = ${newUser.id}`);
...
it("Can use multiple data sources with", async () => {
    ...
    const data = await sqlWith(newUsers)
        .with('bar', extraData)
        .runDB((users) => sql.unsafe`SELECT ${users["*"]}
            , bar.*
            FROM ${users}
...
it("Can handle nested objects", async () => {
    ...
    const data = await sqlWith(newData)
        .runDB((users) => sql.unsafe`
...
it("Can handle dates", async () => {
    const newData = [{
        ...
        first_name: 'John',
        last_name: 'Doe',
        dateOfBirth: new Date(Date.now() - 1000 * 60 * 60 * 24 * 365 * 20 * Math.random()) as any,
    }];
    const data = await sqlWith(newData)
        .runDB((users) => sql.unsafe`

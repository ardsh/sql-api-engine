...
it("Throws an error if view doesn't start with from", async () => {
  expect(() => buildView`SELECT * FROM users`).toThrow(
    'First part of view must be FROM'
  )
})
...
it('Allows building a view from a string', async () => {
  const usersView = buildView`FROM users`
    ...
    .addBooleanFilter(
      'long_email',
      table => sql.fragment`LENGTH(${table._main}.email) > 10`
    )
  ...
  const data = await db.any(sql.unsafe`SELECT * ${usersView} ${whereFragment}`)
  expect(data).toEqual([expect.anything(), expect.anything()])
})

it("Doesn't allow arrays with custom filters", async () => {
  expect(() =>
    buildView`FROM users`.addBooleanFilter(
      ['long_email', 'something_else'],
      table => sql.fragment`LENGTH(${table._main}.email) > 10`
    )
  ).toThrow(
...
it('Allows specifying composite tables', async () => {
  ...
  ON posts.user_id = users.id GROUP BY users.id) combined`
    .addFilters(
      userView.getFilters({
        table: 'combined',
        exclude: ['posts.*']
  ...
  const filters = compositeView.getFilters({
    ...
    include: [
      'combined.users.email',
      'combined.users.first_name',
      'combined.count'
    ]
  })
...
describe('Filters', () => {
  const userView = buildView`FROM users LEFT JOIN posts
    ...
    .addGenericFilter(
      'ID',
      (value: string) => sql.fragment`users.id = ${value}`
    )
    .addInArrayFilter('users.id', table => sql.fragment`${table.users}.id`)
    .addStringFilter(['users.name', 'users.profession'])
    .addComparisonFilter(
      'usersID',
      (table, value) => sql.fragment`${table.users}.id`
    )
    .addComparisonFilter(
      'postsCount',
      ({ users }) => sql.fragment`${users}."authoredPosts"`
    )
  ...
  it('json filter handles a combination of different types', async () => {
    ...
    expect(JSON.parse(query.values[0] as string)).toEqual(settings)
  })

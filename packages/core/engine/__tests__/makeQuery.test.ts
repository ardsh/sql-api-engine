...
describe('withQueryLoader', () => {
  ...
  it('Throws errors for invalid queries', async () => {
    expect(() =>
      makeQueryLoader({
        ...
        query: {
          select: sql.type(zodType)`SELECT *`,
          view: buildView`test_table_bar`
        }
      })
    ).toThrow('First part of view must be FROM')
  ...
  it('Returns virtual fields', async () => {
    ...
    expectTypeOf(loader.getSelectableFields()[0]).toEqualTypeOf<
      'ids' | 'id' | 'uid' | 'value'
    >()
  })
  ...
  it('Returns virtual fields by default if none are selected/excluded', async () => {
    ...
    expect(loader.getSelectableFields()).toContain('ids')
    expectTypeOf(query[0]).toEqualTypeOf<{
      id: number
      ids: string
      uid: string
      value: string
      ...
      virtualFields: {
        ids: {
          resolve: async (row, args) => {
            ...
            expectTypeOf(args.ctx).toEqualTypeOf<
  ...
  it("Warns if there's a semicolon at the end of the select query", async () => {
    ...
    expect(console.warn).toHaveBeenCalledWith(
      expect.stringContaining('Your query includes semicolons')
    )
  ...
  it('Allows ordering by specified columns', async () => {
    ...
    expect(
      (
        await loader.getQuery({
          orderBy: ['value', 'DESC']
        })
      ).sql
    ).toContain(`"value" DESC`)
  })
  ...
  it('Allows ordering by multiple columns', async () => {
    ...
    ).toContain(`"id" ASC, "test_table_bar"."value" DESC`)
    expect(
      (
        await loader.getQuery({
          orderBy: ['value', 'DESC']
  ...
  it('Allows using sql fragment for specifying sortable columns', async () => {
    ...
    ).toContain(`"id" ASC, COALESCE("uid", "value") DESC`)
    expect(
      (
        await loader.getQuery({
          orderBy: ['value', 'DESC']
    ...
    ).toContain(`COALESCE("uid", "value") DESC`)
    expect(
      await loader.load({
        orderBy: [
          ['value', 'DESC'],
  ...
  it('Allows defining filters', async () => {
    const loader = makeQueryLoader({
      ...
      query: {
        select: sql.type(zodType)`SELECT *`,
        view: buildView`FROM test_table_bar`
          .addBooleanFilter('largeIds', sql.fragment`"id" > 5`)
          .addGenericFilter('id', (num: number) => sql.fragment`"id" = ${num}`)
      }
    })
    const query = await loader.getQuery({
      take: 1,
  ...
  it('Selects all fields if select is unspecified', async () => {
    ...
    expectTypeOf(result[0] as InferPayload<typeof loader, any>).toMatchTypeOf<{
      uid: string
      id: number
      value: string
      dummyField: any
  ...
  const filterView = buildView`FROM test_table_bar`
    .addBooleanFilter('largeIds', () => sql.fragment`"id" > 5`)
    .addInArrayFilter('id', () => sql.fragment`"id"`, 'numeric')
    .addInArrayFilter('uid', () => sql.fragment`"uid"`)
    .addDateFilter('date', () => sql.fragment`"date"`)
  const getConditions = filterView.getWhereConditions
  type Filter = Parameters<typeof getConditions>[0]['where']
  ...
  it('Returns minimal count based on next N pages if specified', async () => {
    ...
    expect(query.pageInfo.minimumCount).toEqual(
      query.nodes.length + take * (takeNextPages - 1) + 1
    )
  ...
  it("Doesn't allow filtering by unknown columns", async () => {
    ...
    expectTypeOf(parsed.where).toMatchTypeOf<
      { AND?: any; OR?: any; NOT?: any; id?: any; largeIds?: any } | undefined
    >()
    expectTypeOf(parsed.select?.[0]).toEqualTypeOf<
      'id' | 'value' | 'asdf' | undefined
    >()

    expectTypeOf(
      (parsed as InferArgs<typeof loader>).select?.[0]
    ).toEqualTypeOf<'dummyField' | 'id' | undefined>()
    expectTypeOf(parsed.where).toMatchTypeOf<
      InferArgs<typeof loader>['where'] | undefined
    >()
    expectTypeOf(parsed.orderBy).toMatchTypeOf<
      InferArgs<typeof loader>['orderBy'] | undefined
    >()
    expectTypeOf(parsed.searchAfter).toMatchTypeOf<
      InferArgs<typeof loader>['searchAfter'] | undefined
    >()
  })
  ...
  it('Allows sorting by valid columns', async () => {
    ...
    >()
    expectTypeOf(parsed.select?.[0]).toEqualTypeOf<
      'id' | 'uid' | 'value' | 'dummyField' | undefined
    >()
  })
  ...
  it('Allows sorting with multiple columns', async () => {
    ...
    expectTypeOf(parsed.orderBy?.[0]).toMatchTypeOf<
      ['id' | 'value', string] | 'value' | 'id' | null | undefined
    >()
    expectTypeOf(parsed.select?.[0]).toEqualTypeOf<
      'id' | 'uid' | 'value' | 'dummyField' | undefined
    >()
    ...
    const parser = loader.getLoadArgs({
      transformSortColumns(columns) {
        expectTypeOf(columns).toEqualTypeOf<
          ['value' | 'id', 'ASC' | 'DESC'][] | null | undefined
        >()
  ...
  it('returns all the fields as selectable by default', async () => {
    ...
    expect(selectable).toEqual(
      expect.arrayContaining(['id', 'uid', 'dummyField', 'value'])
    )
    expectTypeOf(selectable[0]).toEqualTypeOf<
      ['id', 'uid', 'value', 'dummyField'][number]
    >()
  ...
  it('returns only the fields that are specified as selectable', async () => {
    ...
    expectTypeOf(selectable).toEqualTypeOf<
      ['id' | 'uid', ...('id' | 'uid')[]]
    >()
  ...
  it('Loads cursor-based with group by', async () => {
    const loader = makeQueryLoader({
      ...
      query: {
        ...
        view: buildView`FROM test_table_bar`.addBooleanFilter(
          'largeIds',
          sql.fragment`"id" > 5`
        ),
  ...
  it('Loads cursor-based even when sorted by complex expression column', async () => {
    ...
    const query = await loader.getQuery(args)
    expect(query.sql).toContain(
      `(UPPER("value") < $1) OR (UPPER("value") = $2 AND "id" < $3)`
    )
    ...
    const parsed = parser.parse(args)
    expect(args).toEqual(parsed)
    expectTypeOf(data.nodes[0]).toEqualTypeOf<
      InferPayload<
        typeof loader,
        {
          select: ['id', 'value']
  ...
  it('Reverses the order when take parameter is negative, while returning items in the original order', async () => {
    ...
    const query = await loader.getQuery(args)
    expect(query.sql).toContain(
      `(UPPER(test_table_bar."value") > $1) OR (UPPER(test_table_bar."value") = $2 AND ("id" < $3 OR "id" IS NULL))`
    )
    expect(query.sql).toContain(
      `ORDER BY UPPER(test_table_bar."value") ASC, "id" DESC`
    )
    const data = await loader.loadPagination(args)
    expect(data).toEqual({
      nodes: [
  ...
  it("Cursor-based pagination doesn't work properly if searchAfter values aren't specified (NOT A BUG) (UNSPECIFIED BEHAVIOR, MAY CHANGE)", async () => {
    ...
    expect(query.sql).toContain(
      `(("id" > $1 OR "id" IS NULL)) OR ("id" = $2 AND TRUE)`
    )
  ...
  it("Cursor-based pagination doesn't work properly if searchAfter values aren't specified for primary column (NOT A BUG) (UNSPECIFIED BEHAVIOR, MAY CHANGE)", async () => {
    ...
    expect(query.sql).toContain(
      `("id" IS NULL) OR ("id" IS NOT NULL AND "value" > $1)`
    )
    expect(query.sql).toContain(`ORDER BY "id" ASC, "value" ASC`)
    expectTypeOf(parsed.where).toEqualTypeOf<undefined>()
    const data = await loader.loadPagination(args)
  ...
  it('Cursor-based pagination tries to work with nullable columns', async () => {
    ...
    expect(
      ...
      `("email" > $1) OR ("email" = $2 AND "date_of_birth" IS NULL) OR ("email" = $3 AND "date_of_birth" IS NOT NULL AND "id" > $4)`
    )
    ...
    expect(
      ...
      `(("date_of_birth" > $1 OR "date_of_birth" IS NULL)) OR ("date_of_birth" = $2 AND "id" > $3)`
    )
  ...
  it('Returns items after cursor', async () => {
    ...
    expect(query.sql).toContain(
      `("value" < $1) OR ("value" = $2 AND "test_table_bar"."id" < $3)`
    )
    expect(query.sql).toContain(
      `ORDER BY "value" DESC, "test_table_bar"."id" DESC`
    )
    expect(args).toEqual(parsed)
    expectTypeOf(
      data.nodes[0] as InferPayload<
  ...
  it('Can access internal columns for determining the cursor', async () => {
    const loader = makeQueryLoader({
      ...
      query: {
        select: sql.type(
          zodType
        )`SELECT test_table_bar.id, test_table_bar.uid, test_table_bar.value`,
        view: buildView`FROM test_table_bar`
      },
      sortableColumns: {
        id: 'id',
  ...
  it('Cursor works with negative take', async () => {
    ...
    expect(query.sql).toContain(
      `(test_table_bar."value" > $1) OR (test_table_bar."value" = $2 AND "id" > $3)`
    )
  ...
  it('distinctOn adds orderBy fragments in front', async () => {
    ...
    expect(query.sql).toContain(`SELECT DISTINCT ON ("test_table_bar"."value")`)
    expect(query.sql).toContain(
      `ORDER BY "test_table_bar"."value" DESC, test_table_bar."id" DESC`
    )
  })

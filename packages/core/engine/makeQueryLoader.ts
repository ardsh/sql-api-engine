...
function isBasicSortFieldOption(
  field?: SortField
): field is
  | string
  | [string, string]
...
function getOrderByDirection(field: OrderField, reverse?: boolean) {
  if (getOrderFieldNullsLast(field[0])) {
    switch (field[1]) {
      case 'ASC':
        return reverse
          ? sql.fragment`DESC NULLS FIRST`
          : sql.fragment`ASC NULLS LAST`
      case 'DESC':
        return reverse
          ? sql.fragment`ASC NULLS FIRST`
          : sql.fragment`DESC NULLS LAST`
    }
  }
  switch (field[1]) {
    case 'ASC':
      return reverse ? sql.fragment`DESC` : sql.fragment`ASC`
    case 'DESC':
      return reverse ? sql.fragment`ASC` : sql.fragment`DESC`
  }
}
...
function interpretOrderBy(field: OrderField, reverse?: boolean) {
  return sql.fragment`${interpretFieldFragment(field[0])} ${getOrderByDirection(
    field,
    reverse
  )}`
}
...
type GetNonEmptyKeys<
  TFilter extends Record<string, z.ZodTypeAny> = never,
  TSortable extends string = never
> =
  | ([TFilter] extends [never] ? 'take' : 'where')
  | ([TSortable] extends [never]
      ? 'take'
      : 'orderBy' | 'searchAfter' | 'takeCursors' | 'cursor' | 'distinctOn')
  | 'take'
  | 'skip'
  | 'select'
  | 'ctx'
...
> = {
  /** The fields that should be included. If `select` is unspecified, all fields are returned. */
   ...
   * ```
   * */
  orderBy?: OptionalArray<
    readonly [TSortable, 'ASC' | 'DESC' | 'ASC NULLS LAST' | 'DESC NULLS LAST']
  > | null
...
> = Pick<
  ...
  [TSelect] extends [never] ? keyof (TVirtuals & z.infer<TObject>) : TSelect
>
...
>(options: {
  ...
  if (
    fromFragment &&
    fromFragment.sql?.length > 5 &&
    !fromFragment?.sql?.match?.(/^\s*FROM/i)
  ) {
    throw new Error('query.view must begin with FROM')
  }
  if (!query?.sql?.match?.(/^\s*(SELECT|WITH)/i)) {
    throw new Error('Your query must begin with SELECT or WITH')
  }
  const type =
    options.type || ((query as QuerySqlToken).parser as z.AnyZodObject)
  ...
  type TFilter = {
    [x in keyof TFilterTypes]?: TFilterTypes[x] extends z.ZodTypeAny
      ? z.infer<TFilterTypes[x]>
      : TFilterTypes[x]
  }
  const interpretFilters = view ? view.getWhereConditions : null
  const sortableAliases = Object.keys(options?.sortableColumns || {}) as [
    TSortable,
    ...TSortable[]
  ]
  ...
  const mapTransformRows = async <T extends z.TypeOf<TObject>>(
    rows: readonly T[],
    args: LoadParameters<
      TFilterTypes,
      TContext,
  ...
  >(
    ...
    let { take, orderBy, select } = allArgs
    const { where, skip, distinctOn, ctx, cursor, takeCursors, searchAfter } =
      allArgs
    const context =
      ctx && options.contextParser?.parse
        ? options.contextParser.parse(ctx)
        : ctx
    ...
    const allConditions = [...auth, ...(filtersCondition || [])].filter(
      notEmpty
    )
    const whereCondition = allConditions.length
      ? sql.fragment`(${sql.join(allConditions, sql.fragment`) AND (`)})`
      : sql.fragment`TRUE`
    let actualQuery = query
    const isPartial = select?.length
    const reverse = !!take && take < 0
    if (take && take < 0) take = -take
    if (reverse && !orderBy?.length) {
      throw new Error(
        'orderBy must be specified when take parameter is negative!'
      )
    }
    const noneSelected = !select?.length
    const distinctFields = Array.isArray(distinctOn)
      ? distinctOn?.map(distinct => sortFields.parse(distinct))
      : distinctOn?.length
      ? [sortFields.parse(distinctOn)]
      : null
    const orderExpressions = Array.isArray(orderBy?.[0])
      ? orderBy?.map(order => orderByWithoutTransform.parse(order))
      : orderBy?.length
      ? [orderByWithoutTransform.parse(orderBy)]
      : distinctFields?.length
      ? []
      : null
    if (distinctFields?.length && !options?.options?.useSqlite) {
      const distinctExpressions = distinctFields.map(field =>
        interpretFieldFragment(sortFieldWithTransform.parse(field))
      )
      const distinctQuery = sql.fragment`SELECT DISTINCT ON (${sql.join(
        distinctExpressions,
        sql.fragment`, `
      )})`
      ...
      for (const expression of distinctFields.reverse()) {
        ...
        const orderPosition = orderExpressions?.findIndex(
          ([field]) => field === expression
        )
        if (typeof orderPosition === 'number' && orderPosition >= 0) {
          orderExpressions?.unshift(
            orderExpressions?.splice(orderPosition, 1)?.[0]
          )
        } else {
    ...
    const zodType: z.ZodType<ResultType<TObject, TVirtuals, TSelect>> =
      isAnyType
        ? type
        : cursorsEnabled
        ? (isPartial ? type.partial() : type).merge(
            z.object({
              [cursorColumns]: z.any()
            })
          )
    ...
    const hasTransformColumns =
      typeof options?.options?.transformColumns === 'function'
    if (hasTransformColumns) {
      select = select.map(
        field => options?.options?.transformColumns?.(field) ?? field
      ) as any[]
    }
    const sqlFields = hasTransformColumns
      ? fields.map(
          field => options?.options?.transformColumns?.(field) ?? field
        )
      : fields
    ...
    if (takeCursors && orderExpressions?.length) {
      if (options.options?.useSqlite) {
        finalKeys.push(
          sql.fragment`json_object(${
            orderExpressions.length
              ? sql.join(
                  orderExpressions.flatMap(expression => [
                    sql.literalValue(expression[0]),
                    interpretFieldFragment(orderByType.parse(expression)[0])
                  ]),
                  sql.fragment`,`
                )
              : sql.fragment``
        ...
        if (!query.sql.includes('lateralcolumns.cursorjson')) {
          ...
          extraSelects.push(
            sql.fragment`lateralcolumns.cursorjson ${sql.identifier([
              cursorColumns
            ])}`
          )
        }
        lateralExpressions.push(
          sql.fragment`jsonb_build_object(${
            orderExpressions.length
              ? sql.join(
                  orderExpressions.flatMap(expression => [
                    sql.literalValue(expression[0]),
    ...
    if ((searchAfter || cursor) && orderExpressions?.length) {
      const orderByExpressions = orderExpressions.map(
        parsed =>
          [
            interpretFieldFragment(orderByType.parse(parsed)[0]),
            parsed[1],
            parsed[0],
            parsed
          ] as const
        ...
        sql.fragment`(${sql.join(
          orderByExpressions.map((_, outerIndex) => {
            ...
            return sql.fragment`(${sql.join(
              expressions.map(
                ([expression, direction, columnAlias], innerIndex) => {
                  let operator = sql.fragment`=`
                  let nullFragment = sql.fragment`TRUE`
                  const orderField = options.sortableColumns?.[columnAlias]
                  const value = searchAfter
                    ? searchAfter[columnAlias]
                    : cursorValues[columnAlias]
                  const isNullable =
                    getOrderFieldNullable(orderField) || value === null
                  ...
                  return value !== null && value !== undefined
                    ? isNullable && isNull
                      ? sql.fragment`(${expression} ${operator} ${value} OR ${nullFragment})`
    ...
    const groupExpression = queryComponents.groupBy
      ? [
          ...(typeof queryComponents.groupBy === 'function'
            ? [queryComponents.groupBy(allArgs)]
            : []),
          ...((queryComponents.groupBy as SqlFragment)?.sql
            ? [queryComponents.groupBy as SqlFragment]
            : []),
          lateralExpressions[0] ? sql.fragment`lateralcolumns.cursorjson` : null
        ].filter(notEmpty)
      : []
    const extraSelectFields = extraSelects.length
    ...
    const baseQuery = sql.type(zodType)`${actualQuery} ${extraSelectFields} ${
      ...
      lateralExpressions[0]
        ? sql.fragment`, LATERAL (SELECT ${sql.join(
            lateralExpressions,
            sql.fragment`, `
          )}) lateralcolumns`
        : sql.fragment``
        ...
        ${
          orderExpressions
            ? sql.fragment`ORDER BY ${sql.join(
                orderExpressions.map(parsed =>
                  interpretOrderBy(orderByType.parse(parsed), reverse)
                ),
                sql.fragment`, `
              )}`
            : sql.fragment``
        ...
        ${
          typeof take === 'number'
            ? sql.fragment`LIMIT ${take}`
            : sql.fragment``
        }
        ${
          typeof skip === 'number'
            ? sql.fragment`OFFSET ${skip}`
            : sql.fragment``
        }
        `
  ...
  const getSelectableFields = () => {
    ...
    ) as [TSelectable, ...TSelectable[]]
    if (selectable?.length) {
      return columns.filter(
        column => selectable.indexOf(column as any) >= 0
      ) as never
    }
    return columns
  }
  ...
  >(
    {
       ...
       * */
      transformSortColumns?: (
        columns?: Array<[TSort, OrderDirection]> | null
      ) => Array<[TSort, OrderDirection]> | null | undefined
    } = {} as never
  ) => {
    ...
    const orderBy = z.preprocess(
      typeof transformSortColumns === 'function'
        ? columns => {
            if (Array.isArray(columns)) {
              if (Array.isArray(columns[0]) || columns[0] === undefined) {
                ...
                return (
                  transformSortColumns([columns].filter(notEmpty) as any) ||
                  columns
                )
              }
            }
        ...
        : a =>
            Array.isArray(a) && (Array.isArray(a[0]) || a[0] === undefined)
              ? a
              : [a].filter(notEmpty),
      orderUnion
    )
    type ActualFilters = [TFilterTypes] extends [never]
      ? never
      : [keyof TFilterTypes] extends [never]
      ? never
      : RecursiveFilterConditions<
          TFilter,
          | Extract<keyof TFiltersDisabled, 'AND' | 'OR' | 'NOT'>
          | TDisabledFilters
        >

    const filterKeys = Object.keys(options.query.view?.getFilters() || {})
    const filterType: any = z
      .lazy(() =>
        z
          .object({
            ...filterKeys.reduce((acc, key) => {
              // TODO: Determine zod type from filter in buildView
              acc[key] = z.any()
              return acc
            }, {} as Record<string, any>),
            ...(!disabledFilters?.OR &&
              options?.options?.orFilterEnabled && { OR: z.array(filterType) }),
    ...
    return z
      .object({
        ...
        searchAfter: sortableColumns.length
          ? z
              .object(
                sortableColumns.reduce(
                  (acc, column) => {
                    acc[column] = z.any()
                    return acc
                  },
  ...
  const self = {
    ...
    async load<TSelect extends keyof (TVirtuals & z.infer<TObject>) = never>(
      args: Pick<
        LoadParameters<
          TFilter,
          z.infer<TContextZod>,
          ...
          TDisabledFilters
        >,
        Exclude<GetNonEmptyKeys<TFilterTypes, TSortable>, 'takeCursors'>
      >,
      ...
      const finalQuery = await getQuery({
        ...args,
        take:
          typeof args.take === 'number' ? args.take : options?.defaults?.take,
        takeCursors: false
      })
      ...
      const load = () =>
        db
          .any(finalQuery)
          .then(async rows => {
            return mapTransformRows(
              reverse ? ((rows as any).reverse() as never) : rows,
              args
            )
          })
          .then(async rows => {
    ...
    >(
      ...
      database?: Pick<CommonQueryMethods, 'any'>
    ): Promise<LoadPaginationResult<ResultType<TObject, TVirtuals, TSelect>>> {
      if (typeof options.contextFactory === 'function') {
        args.ctx = options.contextFactory(args.ctx)
      }
      if (
      ...
      const take =
        typeof args.take === 'number' && args.take < 0 ? -args.take : args.take
      if (
        reverse === -1 &&
        !allArgs.orderBy?.length &&
        !options?.defaults?.orderBy?.length
      ) {
        throw new Error(
          'orderBy must be specified when take parameter is negative!'
        )
      ...
      const extraItems =
        Math.max(Math.min(3, (args?.takeNextPages || 0) - 1), 0) *
          (take || 25) +
        1
      const finalQuery = await getQuery({
        ...args,
        ...
        take:
          (Math.min(
            Math.max(0, take || 100),
            options.options?.maxLimit || 1000
          ) +
            extraItems) *
          reverse
      })
      const countQuery = args.takeCount
        ? sql.type(countQueryType)`SELECT COUNT(*) FROM (${await getQuery({
            ...args,
            skip: undefined,
      ...
      const afterCalls: ((options: {
        result: LoadPaginationResult<any>
        setResult: (
          newResult: PromiseOrValue<LoadPaginationResult<any>>
        ) => void
      }) => void)[] = []
      for (const plugin of options.plugins || []) {
      ...
      const load = () =>
        db
          ...
          .then(async nodes => {
            ...
            const rows =
              reverse === -1 ? (slicedNodes.reverse() as never) : slicedNodes
            const cursors = allArgs.takeCursors && {
              startCursor: toCursor((rows[0] as any)?.[cursorColumns]),
              endCursor: toCursor(
                (rows[rows.length - 1] as any)?.[cursorColumns]
              ),
            ...
            const hasPrevious =
              !!args.skip || !!allArgs.cursor || !!allArgs.searchAfter
      ...
      if (args.takeCount && !countSet) {
        countPromise = db.any(countQuery).then(res => res?.[0]?.count)
      }
      return load()
    }
  }
...
export type InferPayload<
  ...
  TSelect extends Exclude<keyof TResult, number | symbol> = TArgs extends {
    ...
    ? A extends Exclude<keyof TResult, number | symbol>
      ...
      : never
    : never
> = Pick<TResult, [TSelect] extends [never] ? keyof TResult : TSelect>

    ...
    return new Proxy(
        ...
        } as any,
        {
            get: (_target, property: string) => {
                if (property === '*') return sql.fragment`${sql.identifier([tableAlias])}.*`;
...
function getSqlWithArrayQuery<TData extends { [key: string]: SerializableValue }>(data: TData[]) {
    ...
    const types = Object.keys(obj || {}).reduce((acc, key) => {
        if (typeof obj[key] === 'number') return { ...acc, [key]: sql.fragment`numeric` };
        if (typeof obj[key] === 'boolean') return { ...acc, [key]: sql.fragment`bool` };
    ...
    return sql.unsafe`SELECT
        ${sql.join(
            Object.keys(obj).map(
                (key) =>
                    sql.fragment`(x.value->>${sql.literalValue(key)})::${types[key]} AS ${sql.identifier([key])}\n`
            ),
            sql.fragment`, `
        )} FROM JSONB_ARRAY_ELEMENTS(${sql.jsonb(data)}) x`;
}

export function sqlWith<TData extends { [key: string]: SerializableValue }>(data: TData[] = []) {
    const fragments = [{
        name: 'data',
    ...
    const getQuery = (data: TData[]) => {
        ...
        SELECT * FROM ${sql.identifier([fragments[fragments.length - 1]?.name || 'data'])}`;
    };
    const self = {
        as: (
            name: string,
